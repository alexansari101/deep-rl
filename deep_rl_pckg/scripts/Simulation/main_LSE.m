...
if(opt.method==3)
    ...
    opt.beta_t = 1.96;
    opt.eps_band = .01;
    opt.active_lse = ActiveLevelSetEstimation(opt.gp_model, opt.gp_para, xss, opt.level, opt.beta_t, opt.eps_band);
end
...
for k=1:opt.stages %number of iterations
    ...
    if opt.method==3
        opt.active_lse.update(posGP_new, kGP_new);
    end
    ...
    if opt.method==3
        opt.gp.AF = opt.active_lse.utility();
    end
